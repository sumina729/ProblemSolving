...
def sort_depo(list):
    ...
    while tmp:
        ...
        for x, y in tmp:
            if (min_k > cnt_kind(kind_list[y][x])) and (min_k == cnt_kind(kind_list[y][x]) and max_p < power_list[y][x]) and (min_k == cnt_kind(kind_list[y][x]) and max_p == power_list[y][x] and min_y > y) and (min_k == cnt_kind(kind_list[y][x]) and max_p == power_list[y][x] and min_y == y and min_x > x):
                min_k = cnt_kind(kind_list[y][x])
                max_p = power_list[y][x]
                min_y = y
                min_x = x
    ...
    while que:
        ...
        for i in range(4):
            ...
            if -1<nx<N and -1<ny<N and visited[ny][nx] == 0 and kind == kind_list[ny][nx]:
                ...
                if (max_p < power_list[ny][nx]) and (max_p == power_list[ny][nx] and min_y > ny) and (max_p == power_list[ny][nx] and min_y == ny and min_x > nx):
